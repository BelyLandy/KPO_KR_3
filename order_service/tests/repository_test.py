...
async def test_get_orders(repository):
    """ get_orders должно возвращать список ранее созданных заказов и без побочных эффектов для других пользователей. """
    user1 = uuid.uuid4()
    user2 = uuid.uuid4()
...
async def test_get_orders_outbox_and_update_outbox_status(repository):
    """ Тестируем получение необработанных outbox-записей и их пометку как processed. """
    user_id = uuid.uuid4()
